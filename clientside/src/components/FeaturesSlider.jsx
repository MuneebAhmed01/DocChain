...
export default function FeaturesSlider() {
  ...
  return (
    <div className="w-full flex flex-col items-start gap-6 p-6 mt-10">
      {/* Heading */}
      ...
      <div className="w-full">
        ...
        <p className="text-gray-500 max-w-xl mt-2">
          From booking doctors to lab tests and beyond, we bring all your healthcare needs together under one roof — accessible anytime, anywhere
        </p>
      </div>
      {/* Arrows */}
  ...
  {cards.map((card, i) => (
    <div
      ...
      className={`p-5 rounded-3xl transition-all duration-300 border shadow-sm cursor-pointer select-none
        ${active === i ? "bg-blue-600 text-white border-blue-600" : "bg-white text-black border-gray-200"}`}
    >
      <div className={`text-2xl mb-2 ${active === i ? "text-blue-100" : "text-primary"}`}>⌘</div>

      <h2 className={`text-base font-semibold mb-1 ${active === i ? "text-white" : "text-black"}`}>
        {card.title}
      </h2>

      <p className={`text-sm leading-snug ${active === i ? "text-blue-100" : "text-gray-500"}`}>
        {card.desc}
      </p>

      <p className={`underline mt-2 text-sm font-medium ${active === i ? "text-white" : "text-black"}`}>
        Learn More
      </p>
    </div>
  ))}
